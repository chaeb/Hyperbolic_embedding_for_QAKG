...
class QAGNN_Message_Passing(nn.Module):
    def __init__(self, args, k, n_ntype, n_etype, input_size, hidden_size, output_size,
        ...
        self.hyper = hyperboloid.Hyperboloid()
        self.curvature = nn.Parameter(torch.ones([k,1]))  # k x 1



    # message passing helper
    # !! : GQTConvE의 forward에서 curvature를 추가했으므로 여기서도 추가
    def mp_helper(self, _X, edge_index, edge_type, _node_type, _node_feature_extra):
        for i in range(self.k):
            _X = self.gnn_layers[i](_X, edge_index, edge_type, _node_type, _node_feature_extra, self.curvature[i])
            # gnn_layers에서 curvature 추가해서 log, exp 허용하기
...
class QAGNN(nn.Module):
    ...
    def forward(self, sent_vecs, concept_ids, node_type_ids, node_scores, adj_lengths, adj, emb_data=None, cache_output=False):
        ...
        # !! : gnn_input == QA_GNN_MessagePassing의 H
...
class GATConvE(MessagePassing):
    ...
    # !! : curvature 추가
    def forward(self, x, edge_index, edge_type, node_type, node_feature_extra, curvature, return_attention_weights=False):
        # x: [N, emb_dim]
        # edge_index: [2, E]
        ...
        x = self.hyper.logmap0(x, curvature)
        x = torch.cat([x, node_feature_extra], dim=1) # 모든 node를 넣어놓은 것
        x = (x, x)
        # propagate : update(), message() 같이 호출
